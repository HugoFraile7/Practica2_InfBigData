from utils import (
    ...
    extract_sql_to_dataframes,  # <--- ¡AQUÍ!
    extract_json_to_dataframe   # <--- ¡AQUÍ también si usas la función para avisamadrid!
)
import pandas as pd
import pyarrow
import mysql
...
def clean_trafico(df):
    ...
    - Eliminar duplicados
    - Validar que fecha_hora y total_vehiculos no sean nulos
    """
    df = df.copy()
...
def clean_bicimad(df):
    """Limpieza de datos de bicimad:
    - Convertir fecha_hora_inicio y fecha_hora_fin a datetime
    - Eliminar duplicados
    - Validar que usuario_id, estacion_origen, estacion_destino no sean nulos
    """
    df = df.copy()
...
def clean_parkings_merged(df):
    """Limpieza del dataset fusionado de parkings:
    - Convertir fecha a datetime
    - Asegurar tipos numéricos en capacidad_total, plazas, tarifas, latitud/longitud
    - Eliminar duplicados en aparcamiento_id + fecha + hora
    - Validar que no haya nulos en claves principales
    """
    df = df.copy()
...
def clean_avisamadrid(df):
    """Limpieza del dataset de avisos ciudadanos:
    - Convertir fecha_reporte y fecha_resolucion a datetime
    - Eliminar duplicados
    - Validar campos clave no nulos
    """
    df = df.copy()
